...
/**
 * This class implements LogicExpressions using simplified expressions.
 * The methods should return exactly the same values as those in
 * NaivLogicExpressions. So the expressions should be equivalent but simplified.
 *
 * Each method currently returns a random value. This code is obviously only
 * a placeholder so that it compiles and runs, and the methods should look
 * similar to NaivLogicExpressions when you are done.
 *
